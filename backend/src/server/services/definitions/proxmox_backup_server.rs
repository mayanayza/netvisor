use crate::server::hosts::r#impl::ports::PortBase;
use crate::server::services::definitions::{ServiceDefinitionFactory, create_service};
use crate::server::services::r#impl::categories::ServiceCategory;
use crate::server::services::r#impl::definitions::ServiceDefinition;
use crate::server::services::r#impl::patterns::Pattern;

#[derive(Default, Clone, Eq, PartialEq, Hash)]
pub struct ProxmoxBackupServer;

impl ServiceDefinition for ProxmoxBackupServer {
    fn name(&self) -> &'static str {
        "Proxmox Backup Server"
    }
    fn description(&self) -> &'static str {
        "Encrypted and deduplicating backup server for Proxmox VMs, LXCs and hosts"
    }
    fn category(&self) -> ServiceCategory {
        ServiceCategory::Backup
    }

    fn discovery_pattern(&self) -> Pattern<'_> {
        Pattern::AnyOf(vec![
            Pattern::Endpoint(PortBase::new_tcp(8007), "/", "proxmox-backup-gui"),
            Pattern::Port(PortBase::new_tcp(8007)),
        ])
    }

}

inventory::submit!(ServiceDefinitionFactory::new(create_service::<ProxmoxBackupServer>));
